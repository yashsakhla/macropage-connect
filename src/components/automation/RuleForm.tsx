        ...
        <div className="overflow-y-auto flex-1">
          <div className="grid lg:grid-cols-2 gap-6 p-6">
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Rule name</label>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400 dark:text-gray-500">Lower = checked first</span>
                </div>
              </div>

              <div>
                <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">When this happens...</p>
                <TriggerSelector
              ...
              <div className="border border-[#e8ebe8] dark:border-white/10 rounded-xl overflow-hidden">
                <button
                  className="w-full flex items-center justify-between px-4 py-3 text-xs font-semibold text-gray-600 dark:text-gray-400 bg-[#f7f8f6] dark:bg-[#0f1724] hover:bg-[#f0f5f1] dark:hover:bg-emerald-950/30"
                ...
                {showConditions && (
                  <div className="p-4">
                    <p className="text-xs text-gray-500 dark:text-gray-400">Only trigger IF: (conditions builder coming soon)</p>
                  </div>
                )}
              </div>

              <div>
                <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Do this...</p>
              ...
              <div className="border border-[#e8ebe8] dark:border-white/10 rounded-xl overflow-hidden">
                ...
                {showLimits && (
                  <div className="p-4 space-y-3">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-gray-600 dark:text-gray-400">Trigger max</span>
                      <input type="number" className="input h-8 w-16 text-xs text-center" placeholder="∞" />
                      <span className="text-xs text-gray-600 dark:text-gray-400">times per contact</span>
                    </div>
                    <div>
                      <label className="block text-xs text-gray-600 dark:text-gray-400 mb-1">Don't trigger again for:</label>
            ...
            <div className="lg:sticky lg:top-0">
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-3">Preview</p>
              <div className="border border-[#e8ebe8] dark:border-white/10 rounded-2xl overflow-hidden">
                ...
                <div className="bg-[#f7f8f6] dark:bg-[#0f1724] p-4 min-h-36 space-y-3">
                  {previewMessages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.from === 'customer' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[80%] ${msg.from === 'customer' ? 'bg-white dark:bg-[#0b1220] border border-[#e8ebe8] dark:border-white/10' : 'bg-[#e8f5ee] dark:bg-emerald-950/30'} rounded-xl px-3 py-2`}>
                        ...
                        {msg.buttons && msg.buttons.length > 0 && (
                          <div className="mt-2 space-y-1.5 border-t border-[#d0e8d8] pt-2">
                            {msg.buttons.map((b, bi) => (
                              <div key={bi} className="text-center bg-white dark:bg-[#0b1220] text-[#1a5c3a] text-xs rounded-lg px-3 py-1.5 border border-[#c8e6d4]">{b}</div>
                            ))}
                          </div>
                        )}
                      </div>
                  ...
                  {previewMessages.length === 0 && (
                    <p className="text-xs text-gray-400 dark:text-gray-500 text-center pt-4">Configure the rule to see preview</p>
                  )}
                </div>
              </div>

              <div className="bg-[#f7f8f6] dark:bg-[#0f1724] rounded-2xl p-4 mt-4">
                <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">Rule summary</p>
