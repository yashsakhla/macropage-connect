...
export default function WizardStep2Audience({
  ...
  return (
    <div className="space-y-5">
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isSelected ? 'border-[#1a5c3a] bg-[#e8f5ee]' : 'border-[#e8ebe8] hover:border-[#c8e6d4] hover:bg-[#fafffe]'
              )}
            >
              <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-3', isSelected ? 'bg-[#1a5c3a]' : 'bg-[#f7f8f6]')}>
      ...
      {audienceError && !audienceLoading && (
        <div className="border border-amber-200 bg-amber-50 rounded-2xl px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs text-amber-700 flex-1">Could not fetch audience count.</p>
            <button
              onClick={() => refetchAudience()}
              disabled={audienceFetching}
              className="flex-shrink-0 flex items-center gap-1.5 text-xs font-semibold h-7 px-3 rounded-xl bg-white border border-amber-200 text-amber-600 disabled:opacity-50"
            >
      ...
      {/* tag filter */}
      {audienceType === 'tag' && (
        <div className="bg-[#f7f8f6] border border-[#e8ebe8] rounded-2xl p-5 space-y-3">
          <p className="text-sm font-medium text-gray-700">Select tags</p>
          ...
          {tagsError && !tagsLoading && (
            <div className="border border-red-200 bg-red-50 rounded-2xl px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <p className="text-xs text-red-600 flex-1">Could not load tags. We are currently facing an issue.</p>
                <button
                  onClick={() => refetchTags()}
                  disabled={tagsFetching}
                  className="flex-shrink-0 flex items-center gap-1.5 text-xs font-semibold h-7 px-3 rounded-xl bg-white border border-red-200 text-red-600 disabled:opacity-50"
                >
          ...
          <div className="flex flex-wrap gap-2">
            {tagsLoading ? (
              [1, 2, 3, 4].map(i => (
                <div key={i} className="h-8 w-16 bg-gray-100 rounded-xl animate-pulse" />
              ))
            ) : !tagsError && (tagsData ?? []).length === 0 ? (
              <p className="text-xs text-gray-400">No tags found. Add tags to contacts first.</p>
            ) : !tagsError ? (
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive ? 'bg-[#1a5c3a] text-white border-[#1a5c3a]' : 'bg-white text-gray-600 border-[#e8ebe8] hover:border-[#c8e6d4]'
                    )}
                  >
                    {tagName}
                    {tag?.count != null && (
          ...
          {!csvFile ? (
            <div
              ...
              className={cn(
                ...
                isDragging ? 'border-[#1a5c3a] bg-[#e8f5ee]' : 'border-[#e8ebe8] hover:border-[#1a5c3a] hover:bg-[#fafffe]'
              )}
            >
              <input ref={fileInputRef} type="file" accept=".csv" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f) }} />
              <UploadCloud size={40} className="text-gray-300 mx-auto" />
              <p className="text-sm font-medium text-gray-600 mt-3">Drag & drop your CSV file here</p>
              <p className="text-xs text-gray-400 mt-1">or click to browse</p>
              <p className="text-[10px] text-gray-400 mt-4">.csv files only · max 10MB · up to 100,000 contacts</p>
            ...
            <div className="space-y-4">
              <div className="bg-[#e8f5ee] rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">{(csvFile.size / 1024).toFixed(1)} KB · <span className="font-medium text-[#1a5c3a]">{csvRowCount.toLocaleString()} rows detected</span></p>
                </div>
                <button onClick={() => { onCsvFileChange(null); setCsvHeaders([]); setCsvPreviewRows([]); setCsvRowCount(0) }}
                  className="btn-ghost w-7 h-7 text-gray-400">
                  <X size={14} />
                </button>
              </div>
              ...
              {csvHeaders.length > 0 && (
                <div className="bg-white border border-[#e8ebe8] rounded-2xl p-5">
                  <p className="text-sm font-semibold text-gray-800">Map CSV columns</p>
                  <p className="text-xs text-gray-500 mt-0.5">Match your CSV columns to contact fields</p>
                  <div className="mt-4 space-y-2">
                    <div className="grid grid-cols-3 gap-3 text-xs font-semibold text-gray-500 uppercase tracking-wider px-1 mb-2">
                    ...
                    {csvHeaders.map(header => {
                      ...
                      return (
                        <div key={header} className="grid grid-cols-3 gap-3 items-center">
                          <span className="bg-[#f7f8f6] rounded px-2 py-1 text-xs font-mono text-gray-700 truncate">{header}</span>
                          <div className="flex items-center gap-1">
                            ...
                            {mapped === '' && <span className="text-[10px] text-gray-400 flex-shrink-0">ignored</span>}
                          </div>
                          <span className="text-xs text-gray-500 truncate font-mono">{csvPreviewRows[0]?.[header] ?? '—'}</span>
                        </div>
                      )
                    })}
                  </div>
                  ...
                  {csvPreviewRows.length > 0 && (
                    <div className="mt-4 border border-[#e8ebe8] rounded-xl overflow-hidden">
                      ...
                      <p className="text-[10px] text-gray-400 text-center p-2">and {Math.max(0, csvRowCount - 5)} more rows...</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {csvRowCount > 0 && Object.values(csvMapping).includes('phone') && (
                <div className="bg-white border border-[#e8ebe8] rounded-2xl p-4 space-y-2">
                  <div className="flex items-center gap-2 text-sm">
                    <CheckCircle size={15} className="text-[#1a5c3a]" />
                    <span className="text-gray-700">{validCount.toLocaleString()} valid contacts</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <AlertTriangle size={15} className="text-amber-600" />
                    <span className="text-gray-700">{dupCount.toLocaleString()} duplicates removed</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <XCircle size={15} className="text-red-500" />
                    <span className="text-gray-700">{invalidCount.toLocaleString()} invalid numbers removed</span>
                  </div>
                  <div className="pt-2 border-t border-[#f7f8f6]">
                    <p className="text-sm font-semibold text-gray-800">{validCount.toLocaleString()} contacts will receive this campaign</p>
                  </div>
                </div>
              )}
            </div>
      ...
      {!audienceLoading && !audienceError && audienceData && selectedTemplate && audienceType !== 'csv' && (
        <div className="bg-[#f7f8f6] border border-[#e8ebe8] rounded-2xl px-4 py-3">
          ...
          {(() => {
            ...
            return (
              <>
                <div className="h-px bg-[#e8ebe8] mb-2" />
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-gray-500">Rate per conversation</span>
                  <span className="text-xs text-gray-600">{formatINR(cost.perConversation)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-gray-700">Estimated total</span>
