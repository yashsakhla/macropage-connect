  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-screen w-80 bg-white dark:bg-[#0b1220] border-l border-[#e8ebe8] dark:border-white/10 z-40 flex flex-col shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-6">
          ...
          <div>
            ...
            {visibleTags.length === 0 ? (
              <p className="text-xs text-gray-400 dark:text-gray-500 py-2 text-center">No tags found</p>
            ) : (
              <div className="max-h-48 overflow-y-auto space-y-1">
                {visibleTags.map(tag => (
                  <label key={tag} className="flex items-center gap-2.5 py-1.5 cursor-pointer hover:bg-[#f7f8f6] dark:hover:bg-white/5 rounded-lg px-2">
          ...
          <div>
            ...
            <div className="space-y-1">
              {STATUS_OPTIONS.map(opt => (
                <label key={opt.value} className="flex items-center gap-2.5 py-1.5 cursor-pointer hover:bg-[#f7f8f6] dark:hover:bg-white/5 rounded-lg px-2">
                  <input
                    type="radio"
                    name="status"
                  ...
                  <div>
                    ...
                    {opt.desc && <p className="text-[10px] text-gray-400 dark:text-gray-500">{opt.desc}</p>}
                  </div>
                </label>
              ))}
            </div>
          ...
          {/* Date added */}
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Date added</p>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    activeQuick === q
                      ...
                      : 'border-[#e8ebe8] dark:border-white/10 hover:border-[#1a5c3a] hover:text-[#1a5c3a]'
                  )}
                >
                  {q}
                </button>
          ...
          {/* Last seen */}
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Last seen</p>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-[10px] text-gray-400 dark:text-gray-500 mb-1 block">From</label>
                <input
          ...
          {/* Campaigns */}
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Campaigns</p>
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-600 dark:text-gray-400">At least</span>
              <input
                type="number"
                min={0}
        ...
        <div className="px-5 py-4 border-t border-[#e8ebe8] dark:border-white/10 space-y-2">
          <button className="btn btn-primary w-full h-10" onClick={onClose}>
            Apply filters {activeCount > 0 && `(${activeCount})`}
          </button>
          <button
            className="w-full h-9 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 rounded-xl hover:bg-[#f7f8f6] dark:hover:bg-white/5 transition-colors"
            onClick={onClose}
          >
            Cancel
          </button>
