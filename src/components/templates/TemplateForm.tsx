...
import { z } from 'zod'
import { X, Plus, Trash2, Bold, Italic, Strikethrough, Code2, Tag, Globe, Megaphone, ShieldCheck, Wrench } from 'lucide-react'
...
export default function TemplateForm({ onClose, initialData, templateId }: TemplateFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[calc(100vh-48px)] flex flex-col overflow-hidden">
        {/* header */}
        ...
        <form id="template-form" onSubmit={handleSubmit(onSubmit)} className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <div className="bg-white border border-[#e8ebe8] rounded-2xl p-5 space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">Only lowercase letters, numbers, underscores. Cannot be changed after submission.</p>
              </div>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          selected ? 'border-[#1a5c3a] bg-[#fafffe]' : 'border-[#e8ebe8] hover:border-[#c8e6d4]'
                        )}
                      >
              ...
              {/* language */}
              <div>
                <label className="text-sm font-medium text-gray-700 flex items-center gap-1"><Globe size={14} /> Language</label>
            ...
            <div className="bg-white border border-[#e8ebe8] rounded-2xl p-5 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-gray-700">Header (optional)</label>
                  <button type="button" onClick={() => setValue('hasHeader', !values.hasHeader)}
                    className={cn('relative inline-flex h-5 w-9 rounded-full transition-colors', values.hasHeader ? 'bg-[#1a5c3a]' : 'bg-gray-200')}>
                  ...
                  <div className="mt-3 space-y-3">
                    <div className="flex gap-2">
                      {(['TEXT', 'IMAGE', 'DOCUMENT'] as const).map(t => (
                        <button key={t} type="button" onClick={() => setValue('headerType', t)}
                          className={cn('flex-1 border rounded-xl py-2 text-xs font-medium transition-all', values.headerType === t ? 'border-[#1a5c3a] bg-[#e8f5ee] text-[#1a5c3a]' : 'border-[#e8ebe8] text-gray-500 hover:border-[#c8e6d4]')}>
                          {t}
                        </button>
                      ))}
                    </div>
                    {values.headerType === 'TEXT' && (
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 mt-1 text-right">{(values.headerText ?? '').length}/60</p>
                      </div>
                    )}
                    {(values.headerType === 'IMAGE' || values.headerType === 'DOCUMENT') && (
                      <div className="border-2 border-dashed border-[#e8ebe8] rounded-xl p-6 text-center text-sm text-gray-400">
              ...
              <div>
                ...
                <div className="flex gap-1 mt-2 mb-2 flex-wrap">
                  {[['*', 'Bold', <Bold key="b" size={12} />], ['_', 'Italic', <Italic key="i" size={12} />], ['~', 'Strike', <Strikethrough key="s" size={12} />], ['```', 'Code', <Code2 key="c" size={12} />]].map(([wrap, label, icon]) => (
                    <button key={label as string} type="button" onClick={() => insertFormat(wrap as string)}
                      className="bg-[#f7f8f6] text-gray-600 text-xs px-2 py-1 rounded-lg flex items-center gap-1 hover:bg-[#e8ebe8] transition-colors">
                      {icon} {label}
                    </button>
                  ))}
                  <button type="button" onClick={insertVariable}
                    className="bg-[#e8f5ee] text-[#1a5c3a] text-xs px-3 py-1 rounded-lg flex items-center gap-1 font-medium hover:bg-[#d1eedd] transition-colors">
                ...
                <div className="flex justify-between mt-1">
                  ...
                  <p className={cn('text-[10px] text-right ml-auto', (values.body || '').length > 900 ? 'text-red-500' : (values.body || '').length > 800 ? 'text-amber-500' : 'text-gray-400')}>
                    {(values.body || '').length}/1024
                  </p>
                </div>

                {detectedVars.length > 0 && (
                  <div className="bg-[#f7f8f6] rounded-xl p-3 mt-3 space-y-2">
                    <p className="text-[10px] text-gray-500 font-medium">Sample values (shown to Meta during review)</p>
                    {detectedVars.map(v => {
                      const key = v.replace(/\{\{(\d+)\}\}/, '$1')
                      return (
                        <div key={v} className="flex items-center gap-2">
                          <span className="bg-[#e8f5ee] text-[#1a5c3a] text-xs font-mono rounded px-2 py-0.5 min-w-10">{v}</span>
                          <input
              ...
              <div>
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-gray-700">Footer (optional)</label>
                  <button type="button" onClick={() => setValue('hasFooter', !values.hasFooter)}
                    className={cn('relative inline-flex h-5 w-9 rounded-full transition-colors', values.hasFooter ? 'bg-[#1a5c3a]' : 'bg-gray-200')}>
                ...
                {values.hasFooter && (
                  <div className="mt-2">
                    ...
                    <p className="text-[10px] text-gray-400 mt-1 text-right">{(values.footerText ?? '').length}/60</p>
                  </div>
                )}
              </div>
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => setValue('hasButtons', !values.hasButtons)}
                    className={cn('relative inline-flex h-5 w-9 rounded-full transition-colors', values.hasButtons ? 'bg-[#1a5c3a]' : 'bg-gray-200')}>
                    ...
                    <div className="flex gap-2">
                      {[['QUICK_REPLY', 'Quick Replies'], ['CTA', 'Call to Action']].map(([val, label]) => (
                        <button key={val} type="button" onClick={() => setValue('buttonType', val as 'QUICK_REPLY' | 'CTA')}
                          className={cn('flex-1 border rounded-xl py-2 text-xs font-medium transition-all', values.buttonType === val ? 'border-[#1a5c3a] bg-[#e8f5ee] text-[#1a5c3a]' : 'border-[#e8ebe8] text-gray-500')}>
                          {label}
                        </button>
                      ))}
                    </div>
                      ...
                      <div className="space-y-2">
                        {qrFields.map((field, i) => (
                          <div key={field.id} className="flex gap-2 items-center">
                            ...
                            <button type="button" onClick={() => removeQR(i)} className="w-8 h-8 flex items-center justify-center rounded-xl text-red-400 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 size={14} /></button>
                          </div>
                        ))}
                        {qrFields.length < 3 && (
                    ...
                    {values.buttonType === 'CTA' && (
                      <div className="space-y-2">
                        {ctaFields.map((field, i) => (
                          <div key={field.id} className="bg-[#f7f8f6] rounded-xl p-3 space-y-2">
                            <div className="flex gap-2 items-center">
                              ...
                              <button type="button" onClick={() => removeCTA(i)} className="w-8 h-8 flex items-center justify-center rounded-xl text-red-400 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 size={14} /></button>
                            </div>
          ...
          {/* RIGHT — preview */}
          <div className="w-72 border-l border-[#e8ebe8] p-5 flex flex-col gap-4 overflow-y-auto bg-[#f7f8f6]">
            <p className="text-sm font-semibold text-gray-700">Live preview</p>
            <TemplatePreview template={previewTemplate} />

            <div className="bg-white border border-[#e8ebe8] rounded-xl p-4 space-y-2">
              <p className="text-xs font-medium text-gray-700">Character limits</p>
              {values.hasHeader && values.headerType === 'TEXT' && (
                <div>
                  <div className="flex justify-between text-[10px] text-gray-500 mb-1">
                    <span>Header</span><span>{(values.headerText ?? '').length}/60</span>
                  </div>
                  <div className="bg-gray-100 rounded-full h-1"><div className="bg-[#1a5c3a] h-1 rounded-full" style={{ width: `${((values.headerText ?? '').length / 60) * 100}%` }} /></div>
                </div>
              )}
              <div>
                <div className="flex justify-between text-[10px] text-gray-500 mb-1">
              ...
              {values.hasFooter && (
                <div>
                  ...
                  <div className="bg-gray-100 rounded-full h-1"><div className="bg-[#1a5c3a] h-1 rounded-full" style={{ width: `${((values.footerText ?? '').length / 60) * 100}%` }} /></div>
                </div>
              )}
            </div>
          </div>
        ...
        {/* footer */}
        <div className="flex items-center justify-between px-6 py-4 border-t border-[#e8ebe8]">
          <p className="text-xs text-gray-400">Meta approval typically takes 24–48 hours</p>
          <div className="flex gap-2">
            <button
              type="button"
