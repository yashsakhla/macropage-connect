...
export default function ProfileForm({ user, onSave, onAvatarUpload, isSaving }: Props) {
  ...
  // The profile fetch resolves after this form has already mounted with
  // store-only defaults, so re-seed once the full /users/me payload lands.
  useEffect(() => {
    const [first, ...rest] = (user.name ?? '').split(' ')
    reset({
      firstName: first ?? '',
  ...
  }, [user.name, user.bio, user.email, user.phone, user.department, user.jobTitle, user.city])
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="bg-white border border-[#e8ebe8] rounded-2xl p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-6 pt-5 border-t border-[#f5f5f5]">
          ...
          <button type="submit" className="btn-primary h-10 text-sm w-full sm:w-auto" disabled={isSaving || !isDirty}>
