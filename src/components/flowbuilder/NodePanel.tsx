    ...
    <div className="absolute right-0 top-0 h-full w-[300px] bg-white border-l border-[#e8ebe8] z-10 flex flex-col overflow-hidden shadow-xl" style={{ transition: 'transform 0.2s' }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <>
            ...
            {msgTab === 'media' && (
              <div className="space-y-3">
                  ...
                  <div className="flex gap-1 bg-[#f7f8f6] rounded-xl p-1">
                    {(['image', 'video', 'document'] as const).map((m) => (
                      <button
                        ...
                        className={cn('flex-1 py-1 text-2xs font-medium rounded-lg transition-all capitalize', mediaType === m ? 'bg-[#1a5c3a] text-white' : 'text-gray-500')}
                      >
                        {m}
                      </button>
                    ))}
                ...
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1.5">Media URL</label>
                  ...
                  <p className="text-2xs text-gray-400 mt-1">Paste a public URL or media library link for the {mediaType}.</p>
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1.5">Caption (optional)</label>
                  <textarea
            ...
            {msgTab === 'list' && (
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1.5">List button text</label>
                  <input
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-xs font-medium text-gray-700">Sections ({listOptionCount}/10 options)</label>
                  ...
                  <div className="space-y-3">
                    {listSections.map((section, si) => (
                      <div key={section.id} className="border border-[#e8ebe8] rounded-lg p-2 space-y-2">
                        <div className="flex items-center gap-2">
                          <input
                            className="input flex-1 h-7 text-xs"
                          ...
                          />
                          <button onClick={() => removeSection(si)}><X size={11} className="text-gray-400" /></button>
                        </div>
                        <div className="space-y-1.5">
                          {section.rows.map((row, ri) => (
                            <div key={row.id} className="flex items-start gap-1.5 bg-[#f7f8f6] rounded-lg p-1.5">
                              ...
                              <button onClick={() => removeRow(si, ri)} className="mt-1"><X size={11} className="text-gray-400" /></button>
                            </div>
                          ))}
                        </div>
                        <button
                    ...
                    {listSections.length === 0 && (
                      <p className="text-2xs text-gray-400 text-center py-3 border border-dashed border-[#e8ebe8] rounded-lg">
        ...
        {type === 'action' && (
          <>
            <div>
              <label className="text-xs font-medium text-gray-700 block mb-1.5">Action type:</label>
              <select
            ...
            {(data.config?.type ?? 'assign_agent') === 'assign_agent' && (
              <>
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1.5">Assignment strategy:</label>
                  <select
                    className="input w-full h-9 text-xs"
                ...
                {data.config?.strategy === 'specific_agent' && (
                  <div>
                    <label className="text-xs font-medium text-gray-700 block mb-1.5">Agent email or ID:</label>
                    <input
                      className="input w-full h-9 text-xs"
            ...
            {data.config?.type === 'update_field' && (
              <>
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1.5">Field name:</label>
                  <input
                    className="input w-full h-9 text-xs"
                ...
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1.5">New value:</label>
                  <input
                    className="input w-full h-9 text-xs"
            ...
            {data.config?.type === 'mark_resolved' && (
              <p className="text-2xs text-gray-400 bg-[#f7f8f6] rounded-lg p-3">
            ...
            {data.config?.type === 'webhook' && (
              <>
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1.5">Webhook URL:</label>
                  <input
                    className="input w-full h-9 text-xs"
                ...
                <div>
                  <label className="text-xs font-medium text-gray-700 block mb-1.5">Method:</label>
                  <select
                    className="input w-full h-9 text-xs"
            ...
            {data.config?.type === 'jump_to_step' && (
              <div>
                <label className="text-xs font-medium text-gray-700 block mb-1.5">Jump to node:</label>
                <select
                  className="input w-full h-9 text-xs"
