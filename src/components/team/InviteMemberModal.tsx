...
const ROLES: { value: UserRole; icon: React.ElementType; label: string; desc: string; iconBg: string; iconColor: string }[] = [
  ...
  { value: 'manager', icon: Users,      label: 'Manager', desc: 'Manage team, view all conversations',      iconBg: 'bg-blue-50 dark:bg-blue-950/30',   iconColor: 'text-blue-600 dark:text-blue-400'   },
  { value: 'agent',   icon: Headphones, label: 'Agent',   desc: 'Handle assigned conversations',            iconBg: 'bg-[#e8f5ee] dark:bg-emerald-950/30', iconColor: 'text-[#1a5c3a]' },
]
...
export default function InviteMemberModal({ onClose }: InviteMemberModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#0b1220] rounded-2xl w-full max-w-[480px] shadow-2xl flex flex-col max-h-[calc(100vh-32px)] overflow-y-auto">
        <div className="flex items-center gap-3 px-6 py-5 border-b border-[#e8ebe8] dark:border-white/10">
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">They'll receive an email to create their account</p>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-5">
              ...
              {ROLES.map(r => {
                ...
                return (
                  <div key={r.value} onClick={() => setValue('role', r.value as 'admin' | 'manager' | 'agent')}
                    className={cn('border-2 rounded-xl p-3 cursor-pointer transition-all relative', selected ? 'border-[#1a5c3a] bg-[#fafffe] dark:bg-white/5' : 'border-[#e8ebe8] dark:border-white/10 hover:border-[#c8e6d4]')}>
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5 leading-relaxed">{r.desc}</p>
                  </div>
                )
              })}
            </div>
          ...
          <div className="bg-[#f7f8f6] dark:bg-[#0f1724] rounded-xl p-4">
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-3 uppercase tracking-wide">Role permissions</p>
            <div className="space-y-1.5">
              ...
              {perms.cannot.map(p => (
                <div key={p} className="flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500">
          ...
          {/* personal message */}
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5 block">Personal message (optional)</label>
            ...
            <p className="text-[10px] text-gray-400 dark:text-gray-500 text-right mt-1">{watch('message')?.length ?? 0}/200</p>
          </div>
          ...
          <div>
            ...
            {multiOpen && (
              <div className="mt-3">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Paste multiple emails separated by commas</p>
              </div>
            )}
          </div>
        </form>
