...
function ResolutionBar({ pct }: { pct: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">{pct}%</span>
    </div>
  )
}

const STATUS_BADGE = {
  active:   { bg: 'bg-[#e8f5ee] dark:bg-emerald-950/30', text: 'text-[#1a5c3a]', label: 'Online' },
  pending:  { bg: 'bg-amber-50 dark:bg-amber-950/30',  text: 'text-amber-700 dark:text-amber-400',  label: 'Pending' },
  inactive: { bg: 'bg-gray-100 dark:bg-white/10',  text: 'text-gray-500 dark:text-gray-400',   label: 'Offline' },
}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-[#0b1220] border border-[#e8ebe8] dark:border-white/10 rounded-2xl overflow-hidden">
        <div className="grid grid-cols-6 bg-[#f7f8f6] dark:bg-[#0f1724] border-b border-[#e8ebe8] dark:border-white/10 px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
        ...
        {active.map(member => {
          ...
          return (
            <div key={member.id} className="grid grid-cols-6 px-4 py-4 border-t border-[#f5f5f5] items-center hover:bg-[#fafffe] dark:hover:bg-white/5 transition-colors">
              ...
              <div>
                {s ? (
                  <>
                    <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{s.conversationsThisMonth}</p>
              ...
              <div>
                {s ? (
                  <>
                    <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{Math.floor(s.avgResponseTimeSeconds / 60)}m</p>
              ...
              <div>
                {s ? <ResolutionBar pct={s.resolutionRate} /> : <span className="text-gray-300 dark:text-gray-600 text-sm">—</span>}
              </div>
