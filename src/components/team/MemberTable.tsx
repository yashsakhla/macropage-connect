...
function WorkloadBar({ open, max = 20 }: { open: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-medium text-gray-700 dark:text-gray-300">{open} open</span>
    </div>
  )
}
...
function RowMenu({ member, currentUserId, onEdit }: { member: TeamMember; currentUserId: string; onEdit: (m: TeamMember) => void }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button className="w-8 h-8 rounded-lg hover:bg-[#f7f8f6] dark:hover:bg-white/5 flex items-center justify-center text-gray-400 dark:text-gray-500" onClick={() => setOpen(v => !v)}>
        <MoreHorizontal size={14} />
      </button>
      {open && (
        <div className="absolute right-0 top-9 z-50 bg-white dark:bg-[#0b1220] border border-[#e8ebe8] dark:border-white/10 shadow-lg rounded-xl p-1.5 w-52">
          {member.status === 'active' && (
            <>
              ...
              <div className="my-1 h-px bg-[#f5f5f5] dark:bg-white/10" />
...
function MenuItem({ icon: Icon, label, onClick, danger, disabled, disabledTip }: {
  ...
  const colorClass = danger === 'red' ? 'hover:bg-red-50 dark:hover:bg-red-950/30 hover:text-red-600 dark:hover:text-red-400'
    : danger === 'amber' ? 'hover:bg-amber-50 dark:hover:bg-amber-950/30 hover:text-amber-600 dark:hover:text-amber-400'
    : danger === 'green' ? 'hover:bg-[#e8f5ee] dark:hover:bg-emerald-950/30 hover:text-[#1a5c3a]'
    : 'hover:bg-[#f7f8f6] dark:hover:bg-white/5'
  return (
    <button
      disabled={disabled}
      title={disabledTip}
      onClick={e => { e.stopPropagation(); onClick() }}
      className={cn('w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm text-gray-700 dark:text-gray-300 transition-colors text-left', colorClass, disabled && 'opacity-40 cursor-not-allowed')}
    >
      <Icon size={14} />{label}
    </button>
  )
    ...
    <>
          ...
          {ROLE_TABS.map(tab => (
            <button key={tab.value} onClick={() => setRoleFilter(tab.value)}
              className={cn('flex items-center gap-1.5 px-3 h-7 rounded-lg text-xs font-medium transition-all',
                roleFilter === tab.value ? 'bg-[#1a5c3a] text-white' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200')}>
              {tab.label}
              <span className={cn('text-[10px] rounded-full px-1.5', roleFilter === tab.value ? 'bg-white/20 text-white' : 'bg-[#f7f8f6] dark:bg-[#0f1724] text-gray-400 dark:text-gray-500')}>
                {counts[tab.value] ?? 0}
              </span>
            </button>
          ))}
        ...
        <div className="ml-auto flex items-center gap-2">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              className="bg-[#f7f8f6] dark:bg-[#0f1724] border-0 rounded-xl h-9 w-52 pl-8 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a5c3a]/20"
              placeholder="Search members..." />
          </div>
          <select className="bg-white dark:bg-[#0b1220] border border-[#e8ebe8] dark:border-white/10 rounded-xl h-9 px-3 text-sm text-gray-600 dark:text-gray-400 focus:outline-none flex items-center gap-1">
      ...
      <div>
        ...
        {filtered.length === 0 ? (
          ...
          return (
            <div key={member.id}
              className={cn('grid items-center px-5 py-4 border-b border-[#f5f5f5] hover:bg-[#fafffe] dark:hover:bg-white/5 transition-colors', isOwn && 'bg-[#e8f5ee]/20')}
              style={{ gridTemplateColumns: '40px 2fr 1fr 1fr 1fr 1fr 80px' }}>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-gray-900 dark:text-white">{member.name || member.email}</span>
                    {isOwn && <span className="bg-[#e8f5ee] dark:bg-emerald-950/30 text-[#1a5c3a] text-[10px] font-semibold rounded-full px-2 py-0.5">You</span>}
                  </div>
                  <p className="text-xs text-gray-400 dark:text-gray-500">{member.name ? member.email : ''}</p>
                  {member.department && <span className="bg-[#f7f8f6] dark:bg-[#0f1724] text-gray-500 dark:text-gray-400 text-[10px] rounded-full px-2 py-0.5 mt-0.5 inline-block">{member.department}</span>}
                </div>
              </div>
              ...
              <div>
                {member.status === 'active' && (
                  <span className="badge bg-[#e8f5ee] dark:bg-emerald-950/30 text-[#1a5c3a] text-xs">Active</span>
                )}
                {member.status === 'pending' && (
                  <div>
                    <span className="badge bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400 text-xs">Pending invite</span>
                    ...
                    <div className="flex gap-2 mt-1">
                      ...
                      <button className="text-[10px] text-red-500 dark:text-red-400 underline">Cancel</button>
                    </div>
                  </div>
                )}
                {member.status === 'inactive' && (
                  <div>
                    <span className="badge bg-gray-100 dark:bg-white/10 text-gray-500 dark:text-gray-400 text-xs">Inactive</span>
                  </div>
                )}
              </div>
              ...
              <div>
                {member.onlineStatus === 'online' ? (
                  ...
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Invited {member.invitedAt ? fromNow(member.invitedAt) : ''}</p>
                    {member.invitedBy && <p className="text-[10px] text-gray-400 dark:text-gray-500">by {member.invitedBy.name}</p>}
                  </div>
                ) : (
              ...
              <div className="flex items-center justify-end gap-1">
                {member.status !== 'pending' && (
                  <button className="w-8 h-8 rounded-lg hover:bg-[#f7f8f6] dark:hover:bg-white/5 flex items-center justify-center text-gray-500 dark:text-gray-400"
