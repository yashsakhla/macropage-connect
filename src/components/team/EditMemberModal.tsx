    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="flex items-center gap-3 px-6 py-5 border-b border-[#e8ebe8]">
          <p className="text-base font-semibold text-gray-900 flex-1">Edit member</p>
        ...
        {!confirmRemove ? (
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="px-6 py-5 space-y-4 max-h-[65vh] overflow-y-auto">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-gray-400">Click badge to change role</span>
                </div>
              </div>
              ...
              <div>
                ...
                <label className={cn('flex items-center gap-3 border-2 rounded-xl p-4 cursor-pointer transition-all', isActive ? 'border-[#1a5c3a] bg-[#e8f5ee]/20' : 'border-red-200 bg-red-50/20')}>
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-800">{isActive ? 'Active' : 'Inactive'}</p>
                    {!isActive && <p className="text-xs text-red-600 mt-0.5">This will immediately sign them out and revoke access</p>}
                  </div>
                </label>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 px-6 py-4 border-t border-[#e8ebe8]">
              <button type="button" className="text-red-500 text-sm hover:underline flex items-center gap-1 mr-auto" onClick={() => setConfirmRemove(true)}>
          ...
          <div className="px-6 py-6 space-y-4">
            <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-100 rounded-xl">
              ...
              <div>
                <p className="text-sm font-semibold text-red-700">Remove {member.name} from team?</p>
                <p className="text-xs text-red-600 mt-1">Their conversation history remains, but they'll lose all platform access.</p>
              </div>
            </div>
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1.5 block">Type <strong>REMOVE</strong> to confirm</label>
