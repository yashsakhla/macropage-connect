...
const PRIORITIES = [
  { value: 'low', label: 'Low', desc: 'General questions', border: 'border-gray-300', selected: 'border-gray-500 bg-gray-50' },
  { value: 'medium', label: 'Medium', desc: 'Affecting work', border: 'border-amber-200', selected: 'border-amber-500 bg-amber-50' },
  { value: 'high', label: 'High', desc: 'Critical / urgent', border: 'border-red-200', selected: 'border-red-500 bg-red-50' },
] as const
...
export default function SupportTicketForm({ onClose }: Props) {
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center py-8 px-6 text-center">
        <div className="bg-[#e8f5ee] rounded-full p-3">
          <CheckCircle size={32} className="text-[#1a5c3a]" />
        </div>
        <h3 className="text-lg font-bold text-gray-900 mt-4">Ticket submitted! 🎉</h3>
        <p className="font-mono text-sm text-gray-500 mt-1">Ticket #{submitted.ticketNumber}</p>
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="flex items-start justify-between p-6 border-b border-[#e8ebe8]">
        <div>
          <h2 className="text-base font-semibold text-gray-900">Submit a support ticket</h2>
          <p className="text-xs text-gray-500 mt-0.5">We'll reply to your email within 2 hours</p>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        {/* Priority */}
        <div>
          <label className="text-xs font-medium text-gray-700 block mb-1.5">Priority</label>
            ...
            {PRIORITIES.map(p => (
              <button
                ...
                className={`border-2 rounded-xl p-3 text-center transition-colors ${
                  priority === p.value ? p.selected : p.border + ' bg-white'
                }`}
              >
                <p className="text-xs font-semibold text-gray-900">{p.label}</p>
                <p className="text-[0.625rem] text-gray-500 mt-0.5">{p.desc}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex justify-between mt-1">
            ...
            <span className="text-[0.625rem] text-gray-400 ml-auto">{description.length}/2000</span>
          </div>
        </div>

        {/* Attachments */}
        <div>
          <label className="text-xs font-medium text-gray-700 block mb-1.5">Attachments (optional)</label>
          <div
            className="border-dashed border-2 border-[#e8ebe8] rounded-xl p-6 text-center cursor-pointer hover:border-[#c8e6d4] transition-colors"
            onClick={() => fileRef.current?.click()}
            onDragOver={e => e.preventDefault()}
            onDrop={e => { e.preventDefault(); addFiles(e.dataTransfer.files) }}
          >
            <Paperclip size={20} className="text-gray-300 mx-auto mb-2" />
            <p className="text-xs text-gray-400">Attach screenshots or files</p>
            <p className="text-[0.625rem] text-gray-400 mt-0.5">PNG, JPG, PDF up to 5MB</p>
            <button
              type="button"
          ...
          {files.length > 0 && (
            <div className="space-y-2 mt-2">
              {files.map((f, i) => (
                <div key={i} className="flex items-center gap-2 bg-[#f7f8f6] rounded-lg px-3 py-2">
                  <span className="text-xs text-gray-700 flex-1 truncate">{f.name}</span>
                  <span className="text-[0.625rem] text-gray-400">{(f.size / 1024).toFixed(0)}KB</span>
        ...
        <div className="bg-[#f7f8f6] rounded-xl px-4 py-3 flex items-center justify-between">
          <p className="text-sm text-gray-600">Reply to: <span className="font-medium">{user?.email}</span></p>
        </div>
