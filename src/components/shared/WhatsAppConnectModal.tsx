...
export default function WhatsAppConnectModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-xl bg-white dark:bg-[#0b1220] rounded-2xl p-6">
        <div className="flex items-start justify-between">
          ...
          <button onClick={onClose} className="text-gray-500 dark:text-gray-400"><X /></button>
        </div>

        <div className="mt-6 grid grid-cols-3 gap-4">
          <div className="flex flex-col items-center gap-2">
            <div className="w-12 h-12 rounded-full bg-gray-100 dark:bg-white/10 flex items-center justify-center">FB</div>
            <div className="text-sm">Log in to Facebook</div>
          </div>
          <div className="flex flex-col items-center gap-2">
            <div className="w-12 h-12 rounded-full bg-gray-100 dark:bg-white/10 flex items-center justify-center">🏢</div>
        ...
        {showHelp && (
          <div className="mt-4 bg-white dark:bg-[#0b1220] border border-[var(--card-border)] rounded-xl p-4">
            <div className="space-y-3">
              <a className="block text-sm text-blue-600 dark:text-blue-400" href="mailto:support@macropage.in">📧 Email support</a>
