...
function PasswordRequirement({ met, label }: { met: boolean; label: string }) {
  return (
    <div className={cn('flex items-center gap-1.5 text-xs', met ? 'text-[#1a5c3a]' : 'text-gray-400')}>
  ...
  return (
    <SettingsSection title="Security" subtitle="Protect your account with additional security">
          ...
          {([
            ...
            <div key={field}>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPw(p => ({ ...p, [key]: !p[key] }))} className="absolute right-3 top-2.5 text-gray-400">
          ...
          {newPw && (
            <div>
              <div className="flex gap-1 mb-2">
                {[1,2,3,4].map(i => (
                  <div key={i} className={cn('flex-1 h-1.5 rounded-full transition-all', i <= strength ? strength <= 1 ? 'bg-red-400' : strength <= 2 ? 'bg-amber-400' : strength <= 3 ? 'bg-blue-500' : 'bg-[#1a5c3a]' : 'bg-gray-200')} />
                ))}
              </div>
      ...
      <div className="bg-white border border-[#e8ebe8] rounded-2xl p-6 mt-6">
        <p className="text-sm font-semibold text-gray-800 mb-5">Two-factor authentication</p>
