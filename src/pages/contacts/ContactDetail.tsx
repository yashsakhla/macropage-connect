  ...
  if (isError || !contact) {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-64 text-center">
        <p className="text-sm font-semibold text-gray-700 mb-1">Could not load contact</p>
        <p className="text-xs text-gray-400 mb-4">The contact may not exist or there was a network error.</p>
        <button onClick={() => refetch()} className="text-xs font-semibold h-8 px-4 bg-white border border-[#e8ebe8] rounded-xl text-gray-600 hover:text-[#1a5c3a] hover:border-[#c8e6d4] transition-all">
          Try again
        </button>
      </div>
    )
    ...
    <div className="p-6 bg-[#f7f8f6] min-h-screen">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="btn btn-outline h-9 gap-2 flex items-center">
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-1 space-y-4">
          ...
          <div className="bg-white border border-[#e8ebe8] rounded-2xl p-6 text-center">
            ...
            <p className="text-xl font-bold text-gray-900 mt-4">{contact.name}</p>
            ...
            {contact.tags.length === 0 && !editingTags ? (
              ...
              <div className="flex flex-wrap gap-1.5">
                {contact.tags.map(tag => (
                  <span key={tag} className="bg-[#f7f8f6] text-gray-600 text-xs font-medium rounded-full px-2.5 py-1">
                    {tag}
                    {editingTags && (
                      <button className="ml-1 text-gray-400 hover:text-red-500" onClick={() => {}}>×</button>
                    )}
                  </span>
                ))}
                {editingTags && (
          ...
          {Object.keys(contact.customFields).length > 0 && (
            <div className="bg-white border border-[#e8ebe8] rounded-2xl p-5">
              <p className="text-sm font-semibold text-gray-800 mb-3">Custom fields</p>
        ...
        <div className="col-span-2 space-y-4">
          ...
          <div className="bg-white border border-[#e8ebe8] rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-[#e8ebe8]">
              <p className="text-sm font-semibold text-gray-800">Campaign history ({contact.totalCampaigns})</p>
            </div>
            {contact.totalCampaigns === 0 ? (
              ...
              <div className="divide-y divide-[#f5f5f5]">
                {[{ name: 'Diwali Sale 2024', status: 'Delivered', date: '20 Oct 2024' },
                  ...
                  <div key={i} className="flex items-center gap-4 px-5 py-3 hover:bg-[#fafffe] cursor-pointer">
