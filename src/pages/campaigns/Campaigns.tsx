...
export default function Campaigns() {
  ...
  const statCards = [
    { label: 'Total campaigns', value: campaigns.length, icon: Megaphone, bg: 'bg-purple-50', color: 'text-purple-600' },
    { label: 'Messages sent', value: totalSent.toLocaleString(), icon: Send, bg: 'bg-blue-50', color: 'text-blue-600' },
    { label: 'Avg delivery rate', value: `${avgDelivery}%`, icon: TrendingUp, bg: 'bg-[#e8f5ee]', color: 'text-[#1a5c3a]' },
    { label: 'Active now', value: activeCount, icon: Zap, bg: 'bg-amber-50', color: 'text-amber-600' },
  ]
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="input pl-8 w-56 h-9 bg-white"
          ...
          <div className="relative" ref={dateMenuRef}>
            <button
              ...
              className={cn(
                'bg-white border rounded-xl h-9 px-3 flex items-center gap-2 text-sm transition-colors',
                dateRange !== '30d' ? 'border-[#1a5c3a] text-[#1a5c3a]' : 'border-[#e8ebe8] text-gray-600 hover:border-[#c8e6d4]'
              )}
            >
              <Calendar size={14} /> {dateRangeLabel} <ChevronDown size={13} className={cn('transition-transform', showDateMenu && 'rotate-180')} />
            </button>
            {showDateMenu && (
              <div className="absolute right-0 top-10 z-20 bg-white border border-[#e8ebe8] rounded-xl shadow-lg py-1 w-40 text-sm">
                {DATE_RANGE_OPTIONS.map(opt => (
                  <button
                    key={opt.value}
                    onClick={() => { setDateRange(opt.value); setShowDateMenu(false) }}
                    className={cn(
                      'w-full px-3 py-2 text-left hover:bg-[#f7f8f6] transition-colors flex items-center justify-between',
          ...
          <div className="relative" ref={sortMenuRef}>
            <button
              ...
              className="bg-white border border-[#e8ebe8] rounded-xl h-9 px-3 flex items-center gap-2 text-sm text-gray-600 hover:border-[#c8e6d4] transition-colors"
            >
            ...
            {showSortMenu && (
              <div className="absolute right-0 top-10 z-20 bg-white border border-[#e8ebe8] rounded-xl shadow-lg py-1 w-36 text-sm">
          ...
          <div className="flex items-center bg-white border border-[#e8ebe8] rounded-xl p-1">
            <button
              ...
              className={cn('w-7 h-7 flex items-center justify-center rounded-lg transition-all', view === 'list' ? 'bg-[#1a5c3a] text-white' : 'text-gray-400 hover:text-gray-600')}
            >
              <LayoutList size={14} />
            </button>
            <button
              onClick={() => setView('grid')}
              className={cn('w-7 h-7 flex items-center justify-center rounded-lg transition-all', view === 'grid' ? 'bg-[#1a5c3a] text-white' : 'text-gray-400 hover:text-gray-600')}
            >
              <LayoutGrid size={14} />
            </button>
          </div>
...
function EmptyState({ onCreateClick, hasFilter }: { onCreateClick: () => void; hasFilter: boolean }) {
  return (
    <div className="flex flex-col items-center justify-center py-24">
      <div className="w-16 h-16 bg-[#e8f5ee] rounded-2xl flex items-center justify-center mb-4 p-4">
